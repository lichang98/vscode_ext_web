...
testX = np.load(os.path.join(os.path.dirname(model_path), "x_test.npz"))["arr_0"][:100]
colorX = np.load(os.path.join(os.path.dirname(model_path), "colorX.npz"))["arr_0"][:100]
testY = np.load(os.path.join(os.path.dirname(model_path), "y_test.npz"))["arr_0"][:100]
valX = np.load(os.path.join(os.path.dirname(model_path), "x_test.npz"))["arr_0"][:100]
valY = np.load(os.path.join(os.path.dirname(model_path), "y_test.npz"))["arr_0"][:100]
...
acc = model_lib.evaluate(input_model['val_fn'], batch_size=1,num_to_test=100, x_test=testX[:100],y_test=testY[:100])
...
score_norm = model_parser.evaluate(batch_size=1,num_to_test=100,x_test=testX[:100],y_test=testY[:100])
...
# simulate
test_set = {"x_test":testX[:100],"y_test":testY[:100]}
...
for i in range(50):
    ...
    img = np.array(np.squeeze(colorX[i])*255, dtype="uint8")
