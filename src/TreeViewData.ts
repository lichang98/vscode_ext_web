...
// 第一步：创建单项的节点(item)的类
export class TreeItemNodeData extends TreeItem {
    ...
    iconPath = TreeItemNodeData.getIconUriForLabel(this.label);
...
export class TreeViewProviderData implements TreeDataProvider<TreeItemNodeData>{
    // 自动弹出的可以暂不理会
    onDidChangeTreeData?: import("vscode").Event<TreeItemNodeData | null | undefined> | undefined;    
    
    // 自动弹出
    // 获取树视图中的每一项 item,所以要返回 element
    getTreeItem(element: TreeItemNodeData): TreeItem | Thenable<TreeItem> {
        return element;
    }

    // 自动弹出，但是我们要对内容做修改
    // 给每一项都创建一个 TreeItemNode
    getChildren(element?: TreeItemNodeData | undefined): import("vscode").ProviderResult<TreeItemNodeData[]> {
    
        return ['训练数据','测试数据','测试数据标签'].map(
        
            item => new TreeItemNodeData(
                item as string,
    ...
    public static initTreeViewItem(){
        ...
        const treeViewProvider = new TreeViewProviderData();
        
        // registerTreeDataProvider：注册树视图
        // 你可以类比 registerCommand(上面注册 Hello World)
        window.registerTreeDataProvider('data',treeViewProvider);
    }
}
