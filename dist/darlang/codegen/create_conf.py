...
def create_config(path: str, snn: SNN, output_path):
    ...
    while board_id <= board_num:
        ...
        while childboard_id <= childboard_num:
            ...
            if (layer_id >= layer_num):
                ...
                fw = open(os.path.join(output_path,'linkout'), 'wb')
            ...
            str2 = 'layerWidth' + ID
            fw = open(os.path.join(output_path,str2), 'wb')
            pickle.dump(layerWidth, fw)
            fw.close()

            str3 = 'nodelist' + ID
            fw = open(os.path.join(output_path, str3), 'wb')
            pickle.dump(nodelist, fw)
            fw.close()

            c_head = '40000'
            str4 = os.path.join(ID + "clear.txt")
            f = open(os.path.join(output_path, str4), "w")
            ...
            f = open(os.path.join(output_path, str5), "w")
            for i in range(id2 - id1):
                # print("i:", i)
                for x, y in nodelist[i]:
                    tmp = get_headpack(x, y)
