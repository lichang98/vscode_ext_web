    ...
    def run(self, x_test=None, y_test=None, dataflow=None, **kwargs):
        ...
        for batch_idx in range(num_batches):
            ...
            if 'v_thresh' in kwargs:
                data_batch_kwargs['v_thresh'] = kwargs['v_thresh']
            ...
            if kwargs["is_obj_detection"]:
                truth_d += list(np.squeeze(y_b_l))
                guesses_d += list(output_b_l_t)
            else:
    ...
    def setup_layers(self, batch_shape):
        ...
        for layer in self.parsed_model.layers[1:]:
            ...
            if layer_type == 'Concatenate':
                self.add_concatenate_layer(layer)
            else:
            ...
            if layer_type == 'Dense':
                ...
                self.build_upsampling2D(layer)
            elif layer_type == 'Concatenate':
                self.build_concatenate(layer)
...
def build_upsampling2D(layer, delay):
    """
    Building Upsampling2D
    """
    print("Tmp, calculating Upsampling2D connections....")
    _, h,w,channel = layer.output_shape
    prev_w = int(w//2)
    prev_h = int(h//2)
    connections = []
    for i in range(int(prev_w*prev_h)):
        prev_pos = i*channel
        next_pos1 = (i//prev_w*4*prev_w+(i%prev_w)*2)*channel
        next_pos2 = (i//prev_w*4*prev_w+(i%prev_w)*2+1)*channel
        next_pos3 = (i//prev_w*4*prev_w+(i%prev_w)*2+2*prev_w)*channel
        next_pos4 = (i//prev_w*4*prev_w+(i%prev_w)*2+2*prev_w+1)
        for idx in range(channel):
            connections.append((prev_pos+idx, next_pos1+idx, 1, delay))
            connections.append((prev_pos+idx, next_pos2+idx, 1, delay))
            connections.append((prev_pos+idx, next_pos3+idx, 1, delay))
            connections.append((prev_pos+idx, next_pos4+idx, 1, delay))

    # prev_offset = int(prev_w*prev_h)
    # curr_offset = int(w*h)
    # for fan_out in range(channel):
    #     for i in range(int(prev_w*prev_h)):
    #         prev_pos = i+fan_out*prev_offset
    #         next_pos1 = i//prev_w*4*prev_w+(i%prev_w)*2
    #         next_pos2 = i//prev_w*4*prev_w+(i%prev_w)*2+1
    #         next_pos3 = i//prev_w*4*prev_w+(i%prev_w)*2+2*prev_w
    #         next_pos4 = i//prev_w*4*prev_w+(i%prev_w)*2+2*prev_w+1
    #         connections.append((prev_pos, next_pos1+curr_offset*fan_out, 1, delay))
    #         connections.append((prev_pos, next_pos2+curr_offset*fan_out, 1, delay))
    #         connections.append((prev_pos, next_pos3+curr_offset*fan_out, 1, delay))
    #         connections.append((prev_pos, next_pos4+curr_offset*fan_out, 1, delay))
    return connections


def build_concatenate_layer(layer, delay):
    """
    Building Concatenate layer

    Parameters
    ----------
    layer: ANN layer
    delay: the neuron config delay

    Returns
    -------
    Tuple contains two connections according two previous layers of concatenate layer
    """
    prev_layer1 = layer.inbound_nodes[0].inbound_layers[0]
    prev_layer2 = layer.inbound_nodes[0].inbound_layers[1]
    print("concatenate layer connect prev 1={}, prev_2={}".format(prev_layer1.name, prev_layer2.name))
    _, h1, w1, c1 = prev_layer1.output_shape
    _, h2, w2,c2 = prev_layer2.output_shape
    conns1, conns2 = [], []
    idx1, idx2, idx_total=0,0,0
    for i in range(h1):
        for j in range(w1):
            # connection of prev layer 1
            for k in range(c1):
                conns1.append((idx1, idx_total, 1, delay))
                idx1 +=1
                idx_total +=1
            
            # connection of prev layer 2
            for k in range(c2):
                conns2.append((idx2, idx_total, 1, delay))
                idx2 += 1
                idx_total += 1
    # for idx in range(h1*w1*c1):
    #     conns1.append((idx, idx, 1, delay))

    # offset_next = h1*w1*c1
    # for idx in range(h2*w2*c2):
    #     conns2.append((idx, idx+offset_next, 1, delay))
    
    return (conns1, conns2)



def build_concatenate(layer,delay):
    """
    Building Concatenate layer
    """
    prev_layer1 = layer.inbound_nodes[0].inbound_layers[0]
