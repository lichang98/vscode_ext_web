...
def fixpt(weights, bit_width=8):
    ...
    max_pos_wts = np.array([np.max(wt) for wt in weights])
    min_pos_wts = np.array([np.min(wt[wt >=0]) for wt in weights if len(wt[wt >=0]) > 0])
    near0_neg_wts = np.array([np.max(wt[wt <0]) for wt in weights if len(wt[wt < 0]) > 0])
    far0_neg_wts = np.array([np.min(wt[wt < 0]) for wt in weights if len(wt[wt < 0]) > 0])
    scale_facs_pos = range_pos / (max_pos_wts - min_pos_wts)
    scale_facs_neg = range_neg / (far0_neg_wts - near0_neg_wts)
    for i in range(len(weights)):
        weights[i][weights[i] >=0 ] = scale_facs_pos[i]* (weights[i][weights[i] >=0 ] - min_pos_wts[i])
        weights[i][weights[i] < 0] = scale_facs_neg[i] * (weights[i][weights[i] < 0]-near0_neg_wts[i])
        weights[i] = np.floor(weights[i])
...
#                                     np.min(spiking_model.connections[i].w)))
# spiking_model.snn.store()
# print("After converting weights to uint8 range....")
# # TODO v_thresh with fix point for each layer seems good
# test_set['v_thresh'] = sys_param_vthresh
# accu = spiking_model.run(**test_set)
# print("accu={}".format(accu))
# print("weights max={}, min={}".format([np.max(list(e.w)) for e in spiking_model.connections],
#                                     [np.min(list(e.w)) for e in spiking_model.connections]))

# ##############br2 pkl file generating######################
# print("saving br2.pkl.......")
# br2_model = {
#     'neurons':[e.N for e in spiking_model.layers],
#     'synapses_i': [list(e.i) for e in spiking_model.connections],
#     'synapses_j': [list(e.j) for e in spiking_model.connections],
#     'synapses_w': [list(e.w) for e in spiking_model.connections],
#     'v_thresh':1,
#     'run_dura': 100
# }
...
# print("saving darwinlang files")
# snn_model_darlang = {
#     'projectName': 'segmentation',
#     'version': '0.0.1',
#     'target':'darwin2',
#     'netDepth': len(spiking_model.layers),
#     'delayType':[0,0],
...
# # save weights files, each synapses a file, [(src index, dest index, weight, delay),...]
# print("save weights file, total {} synapses".format(len(spiking_model.connections)),flush=True)
# for i in range(len(spiking_model.connections)):
#     info = list(zip(spiking_model.connections[i].i, spiking_model.connections[i].j, spiking_model.connections[i].w, [1]*len(spiking_model.connections[i].w)))
#     info = np.array(info)
...
# print("Converting darwinlang json file to binary file.....")
...
for i in range(len(spiking_model.connections)):
    ...
    all_wts.append(np.array(spiking_model.connections[i].w, dtype="float32"))
    br2_synapses[-1].w = np.array(spiking_model.connections[i].w, dtype="float64")
...
br2_net = brian2.Network(br2_neurons, br2_synapses, br2_monitor)
...
########Image classification searching##############
# all_accus=[]
# v_th_range=list(range(1,24,2))
...
#########################
################Segmentation searching#########

def get_output_b_l(spike_trains:dict):
    output_b_l = np.zeros((64,64,2), dtype="int32")
    outputs = list(spike_trains.items())
    outputs = list(sorted(outputs, key=lambda x: int(x[0])))
    mid = int(len(outputs)//2)
    for i in range(0, mid):
        len1 = len(outputs[i][1])
        len2 = len(outputs[i+mid][1])
        if len2 > len1:
            output_b_l[int(i/64), int(i%64), 1]=1
        else:
            output_b_l[int(i/64), int(i%64), 0]=1
    # for i in range(0, len(outputs), 2):
    #     len1 = len(outputs[i][1])
    #     len2 = len(outputs[i+1][1])
    #     if len2 > len1:
    #         output_b_l[int(i/2/64), int((i/2)%64), 1]=1
    #     else:
    #         output_b_l[int(i/2/64), int((i/2)%64), 0]=1
    
    return output_b_l

# all_accus = []
# v_th_range = list(range(1,32,2))
# for v_th in v_th_range:
#     acc =0
#     for i in range(50):
#         print("---test sample {}, shape={}".format(i, np.shape(testX[i])), end='')
#         sample = testX[i].flatten()/brian2.ms
#         br2_neurons[0].bias = sample
#         br2_net.run(100*brian2.ms, namespace={'v_thresh': v_th},report=None)
#         output_spike = br2_monitor.spike_trains()
#         pred_lbs = get_output_b_l(output_spike)
#         print(", true_lbs shape={}, pred lbs shape={}".format(np.shape(testY[i]), np.shape(pred_lbs)), end='')
#         tmp_accu = np.mean(pred_lbs == testY[i])
#         print(", accu={}".format(tmp_accu))
#         acc += tmp_accu
#         br2_net.restore()
    
#     print("searching vth={}, accu={}".format(v_th, acc/50.0))
#     all_accus.append([acc/50.0, "v_th={}".format(v_th)])

# print("all_accs={}".format(all_accus))
...
#     output_spike = br2_monitor.spike_trains()
#     pred_lbs = get_output_b_l(output_spike)
#     pred_mask = np.zeros((64,64),dtype='uint8')
#     pred_mask[pred_lbs[:,:, 1] == 1] = 255
#     print("sum of pred_mask={}".format(np.sum(pred_mask)))
#     Image.fromarray(pred_mask).save("/data/lc/sample_{}.png".format(i))
#     print("testx shape={},testy shape={}, pred_lbs shape={}".format(np.shape(testX[i]),np.shape(testY[i]), np.shape(pred_lbs)))
#     Image.fromarray(np.array(np.squeeze(testX[i])*255, dtype='uint8')).save("/data/lc/sample_true_{}.png".format(i))
#     print("processing sample image {} finish. accu={}".format(i, np.mean(pred_lbs == testY[i])))
#     br2_net.restore()
    # input_spike_arrs=[]
